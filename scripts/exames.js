  ...
  return `
    <div class="exame-item">
      ...
      <div class="exame-actions">
        <button class="edit-btn" data-action="edit" data-id="${exame.id}">
          <i class="fas fa-edit"></i> Editar
        </button>
        <button class="delete-btn" data-action="delete" data-id="${exame.id}">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Add event delegation for exam action buttons
  document.addEventListener('click', function(event) {
    if (event.target.matches('[data-action="edit"]') || event.target.closest('[data-action="edit"]')) {
      const button = event.target.matches('[data-action="edit"]') ? event.target : event.target.closest('[data-action="edit"]');
      const id = parseInt(button.dataset.id);
      editExame(id);
    } else if (event.target.matches('[data-action="delete"]') || event.target.closest('[data-action="delete"]')) {
      const button = event.target.matches('[data-action="delete"]') ? event.target : event.target.closest('[data-action="delete"]');
      const id = parseInt(button.dataset.id);
      deleteExame(id);
    }
  });
